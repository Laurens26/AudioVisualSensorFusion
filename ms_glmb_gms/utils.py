...
def kalman_update_multiple_2(z, model, s, m, P):
    plength = m.shape[1]
    zlength = z.shape[1]
    ...
    for idxp in range(0, plength):
        qz_temp, m_temp, P_temp = kalman_update_single(z, model.H[s], model.R[s], m[:, idxp], P[:, :, idxp])
        qz_update[idxp, :] = qz_temp
        m_update[:, idxp, :] = m_temp
        P_update[:, :, idxp] = P_temp
...
def kalman_msjointupdate_multiple(Z, k, nestmeasidxs, model, m, P):
    ...
    qz_temp, m_temp, P_temp = kalman_update_single(stacked_z, stacked_H, stacked_R, m, P)
...
def gibbswrap_jointpredupdt_custom(P0, m):
    ...
    for sol in range(1, m):
        for var in range(0, n1):
            ...
            currsoln[var] = np.digitize(np.random.rand(1), np.cumsum(tempsamp) / sum(tempsamp))
...
if __name__ == '__main__':
    ...
    siz = np.array([4, 15, 9, 9])
    idx = np.array([[2, 1, 1, 0]])
    ndsub2ind(siz, idx)